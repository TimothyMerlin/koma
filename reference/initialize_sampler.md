...
This Cholesky factor, along with the gamma parameter for equation \\j\\,
is returned. The Cholesky factor is used to draw the candidate gamma in
the MH algorithm.
...
(`gamma_parameters_j`) and the Cholesky factor of the inverse of the
Hessian (`cholesky_of_inverse_hessian`) if `number_endogenous_in_j` is
greater than 0. If not, only the `gamma_parameters_j` parameter is
returned as 0.
