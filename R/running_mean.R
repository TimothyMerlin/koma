...
#' @param ... Additional arguments controlling the output. See Details.
#'
#' @details
...
#'     include.}
#'   \item{params}{Optional character vector of parameter groups to include
...
#'   \item{max_draws}{Optional integer cap on the number of draws returned.
#'     When set, the most recent draws are kept.}
#'   \item{grace_draws}{Optional integer number of initial retained draws to
#'     flag as a grace window for convergence interpretation. If NULL, defaults
#'     to `max(50, ceiling(0.1 * n_retained))` per series.}
#' }
#'
#' Note: \code{sigma} values are based on \code{omega_tilde_jw} and use only
...
#' @export
running_mean.koma_estimate <- function(x, ...) {
  ...
  if (!is.null(grace_draws)) {
    if (!is.numeric(grace_draws) || length(grace_draws) != 1L || is.na(grace_draws)) {
      cli::cli_abort("`grace_draws` must be a single numeric value.")
    }
    grace_draws <- as.integer(grace_draws)
    if (!is.finite(grace_draws) || grace_draws < 0L) {
      cli::cli_abort("`grace_draws` must be a non-negative integer.")
    }
  }
  ...
  if (!length(params)) {
    cli::cli_abort("No parameter groups selected.")
  }

  sys_eq <- x$sys_eq

  running_mean_matrix <- function(mat) {
    out <- matrix(NA_real_, nrow = nrow(mat), ncol = ncol(mat))
    denom <- seq_len(ncol(mat))
    for (jx in seq_len(nrow(mat))) {
      out[jx, ] <- cumsum(mat[jx, ]) / denom
    }
    out
  }
  ...
  build_draw_df <- function(draws, variable, param_name, coef_names,
    ...
    mat <- prepared$mat
    draw_idx_full <- prepared$draw_idx
    coef_names <- normalize_coef_names(coef_names, nrow(mat))

    mean_mat <- running_mean_matrix(mat)
    n_retained <- ncol(mean_mat)
    grace_n <- if (is.null(grace_draws)) {
      max(50L, as.integer(ceiling(0.1 * n_retained)))
    } else {
      grace_draws
    }
    grace_n <- min(grace_n, n_retained)

    keep_pos <- seq_len(n_retained)
    if (!is.null(max_draws) && length(keep_pos) > max_draws) {
      keep_pos <- utils::tail(keep_pos, max_draws)
    }

    mean_mat <- mean_mat[, keep_pos, drop = FALSE]
    draw_idx <- draw_idx_full[keep_pos]

    values <- as.vector(mean_mat)
    ...
    pos_rep <- rep(keep_pos, each = nrow(mean_mat))
    df <- data.frame(
      draw = rep(draw_idx, each = nrow(mean_mat)),
      value = values,
      variable = variable,
      param = param_name,
      coef = coef_rep,
      draw_position = pos_rep,
      in_grace_window = pos_rep <= grace_n,
  ...
  for (variable in variables) {
    ...
    df_sigma <- add_draws(
      ...
      variable = variable,
      build_draw_df = build_draw_df,
      # Use only the variance element from each covariance draw.
  ...
  if (!length(df_list)) {
    cli::cli_abort("No draws available after filtering.")
  }
...
#'
#' @param x A `koma_estimate` object.
#' @param ... Additional arguments controlling the plot. See Details in
#'   [running_mean()].
#'
#' @details
...
running_mean_plot <- function(x, ...) {
  UseMethod("running_mean_plot")
}

#' @export
running_mean_plot.koma_estimate <- function(x, ...) {
  stopifnot(inherits(x, "koma_estimate"))
  ...
  rm_args <- args
  rm_args$interactive <- NULL
  rm_args$facet_ncol <- NULL
  rm_args$scales <- NULL
  df_long <- do.call(running_mean, c(list(x = x), rm_args))

  shade_list <- lapply(split(df_long, df_long$label), function(d) {
    d_grace <- d[d$in_grace_window, , drop = FALSE]
    if (!nrow(d_grace)) {
      return(NULL)
    }
    data.frame(
      label = d$label[1],
      xmin = min(d$draw),
      xmax = max(d_grace$draw),
      stringsAsFactors = FALSE
    )
  })
  shade_list <- Filter(Negate(is.null), shade_list)
  shade_df <- if (length(shade_list)) do.call(rbind, shade_list) else NULL
  ...
  fig <- fig +
    ggplot2::geom_line(color = "#d95f02", linewidth = 0.4, alpha = 0.8) +
    ggplot2::facet_wrap(~label, scales = scales, ncol = facet_ncol) +
    ggplot2::labs(x = "Draw", y = "Running mean") +
