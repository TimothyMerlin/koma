...
* Added `dummies(prefix, spec)` equation syntax, e.g. `dummies(covid, 1:8)`, as shorthand for a run of indexed dummy variables (`covid_1+covid_2+...+covid_8`). `spec` uses the same range/list syntax as lag notation. It's expanded before validation, so the expanded names must still be declared in `exogenous_variables` like any other regressor; see `vignette("koma-equations")`.
